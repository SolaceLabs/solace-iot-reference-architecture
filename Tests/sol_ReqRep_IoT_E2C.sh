...
generateReplyList ${STREAM_COUNT}
echo ""
echo ""
...
${SDKPERF_HOME}/${SDKPERF_JAVA} -cip=${SOLACE_CORE} -q  -cc=${STREAM_COUNT} -jcf=cf \
 -stl=${topicList} -nmpr=1 -portl=${replyList}  -sri=100 -cor |  grep -E '^Total.*rec|^Computed subscriber rate' > /tmp/sdkperf &

sleep 5

resultPub=`${SDKPERF_HOME}/${SDKPERF_MQTT} -cip=${SOLACE_EDGE} -cc=${STREAM_COUNT} -q \
 -ptl=${topicList} -stl=${replyList} -mpq=0 -msa=${1} -mr=${msgRate} -mn=${msgNum} -mrt=max\
